...
// Test_GzipRemovesContentLength ensures a Content-Length set by the inner
// handler (http.ServeContent for static files does this) is dropped when the
// body gets compressed: it refers to the uncompressed size and would truncate
// the response on the client side.
func Test_GzipRemovesContentLength(t *testing.T) {
	payload := strings.Repeat("static file content ", 200)
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Length", strconv.Itoa(len(payload)))
		_, _ = w.Write([]byte(payload))
	})
	srv := httptest.NewServer(compressResponse(inner))
	defer srv.Close()

	// the default client advertises gzip and transparently decompresses
